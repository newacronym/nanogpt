...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        # its kind of a mask than a bias
        self.register_buffer("bias", torch.tril(torch.ones(config.block_size, config.block_size)).view(1, 1, config.block_size, config.block_size))
    
    def forward(self, x):
        B, T, C = x.size() # batch_size, sequence length, embedding dimensionality
        # calculate query, key, values for all heads in batch and moved head forward to be the batch
        # nh is "number of heads", hs is "head size" and C (channels) = nh*ns
        qkv = self.c_attn(x)
...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        # the token embedding and the top lm_head share the same weights according to the paper
        self.transformer.wte.weight = self.lm_head.weight

        # init params
        # apply the init_weights function on all the modules inside nn
...
class DataLoaderLite:
    ...
    def reset(self):
        ...
        # we are striding for each processes
...
model.to(device)
use_compile = False # TO bugfix: it interferes with the evaluation  
if use_compile:
...
# using lr decay and steps according to GPT3 paper
max_lr = 6e-4
min_lr = max_lr * 0.1
warmup_steps = 715
max_steps = 19073
...
for step in range(max_steps):
    ...
    # once in a while we generate from the model (except step 0, which is just noise)
    # this code works fine if we disable torch.compile
    if ((step > 0 and step % 250 == 0) or last_step) and (not use_compile):
        model.eval()
        num_return_sequences = 4
        max_length = 32
        tokens = enc.encode("Hello, I'm a langugae model,")
    ...
    for micro_step in range(grad_accum_steps):
        ...
        # instead of SUM we want MEAN. Scale the loss here.
        loss = loss / grad_accum_steps
        loss_accum += loss.detach()
        # what this does is, it doesnt allow gradient sync to happen between all the node
        # until the very last step of gradient accumulation
        if ddp:
            model.require_backward_grad_sync = (micro_step == grad_accum_steps - 1)
        loss.backward()
    # it does all reduce on loss_accum for all the processes and does the average
    # distributed those losses around all the processes
    if ddp:
    ...
    if master_process:
        print(f"step {step:4d}, loss:{loss_accum.item():.6f}, lr: {lr:.4f} norm: {norm:.4f} | dt: {dt:.2f}ms, tok/sec: {tokens_per_sec}")
        with open(log_file, "a") as f:
